...
**Update — financial selection, operational explanation**: ADR-0041 supersedes the full-tree prompt policy above. The backend now selects only material Posting Activity Accounts by financial impact before prompting; their Driver Formula terms are retained as the operational evidence the LLM must use to explain every selected item.

**Update — output shape**: one headline sentence plus zero to four bullets: exactly one for every ADR-0041-selected contributor. A diffuse movement has no selected contributor and therefore no bullets.
