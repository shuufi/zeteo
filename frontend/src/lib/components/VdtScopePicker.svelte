<script lang="ts">
  ...
  const candidates = $derived(
    Object.values(vdtStore.tree).filter((n) => n.nodeType !== 'Driver Formula' && n.nodeType !== 'Driver'),
  );
  ...
  const selectedLabel = $derived(value ? (vdtStore.tree[value]?.name ?? value) : '');
...
<div class="flex items-center gap-1.5" bind:this={rootEl}>
  <span class="text-xs whitespace-nowrap text-gray-500 dark:text-gray-400">Scope:</span>
